"""
    ./py/plot_partition_of_unity.py

    Author: Fabian R. Lux
    Date:   2023-03-22

    Plots the partition of unity which is used to create the Y junction in the manuscript.
"""
...
for i in range(n_r):
    for j in range(n_phi):
        ...
        for region in range(3):
            chis[region, i, j] = smooth_phase(z, ell, phi=phi, region=region)

fig, axs = plt.subplots(1, 3, subplot_kw=dict(
    projection="polar"), figsize=(2, 0.8), dpi=300, layout='constrained')
deg = 180 / np.pi
...
sk = 1
fig.colorbar(ctf, ax=axs.ravel().tolist(), orientation='vertical',
             ticks=[0, 0.5, 1], shrink=sk, aspect=5*sk, pad=0.1)
